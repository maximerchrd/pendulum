...
class IMUDistPendulumEnv(gym.Env):
    def __init__(self):
        super(IMUDistPendulumEnv, self).__init__()
        # Inputs: [x_pos, x_velocity, Angle, Angle_Velocity]
        self.high = np.array([0.0, 0.0, 0.42, 5.0], dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        return np.array([self.state[0], self.state[1], self.state[2], self.state[3]], dtype=np.float32), {}
    ...
    def step(self, action):
        ...
        return np.array([x, x_dot, theta, theta_dot], dtype=np.float32), reward, terminated, False, {}

if __name__ == "__main__":
    # 1. TRAIN (PyTorch)
    env = IMUDistPendulumEnv()
    print("🚀 Starting Training...")
    
    # SB3 Default PPO uses Tanh activation and two layers of 64 neurons
    model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.0003)
    model.learn(total_timesteps=300000) # Reduced for testing, increase to 800k for real result
    ...
    test_in = np.array([[0.1, 0.05, 0.1, 0.5]], dtype=np.float32)
